  ...
  return (
    <div className="w-full bg-white  rounded-lg shadow-sm border border-gray-200 ">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-6 border-b border-gray-200 ">
        <h2 className="text-xl font-semibold text-gray-900 ">Finance & Budgeting</h2>
          ...
          {isCustomCurrency ? (
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-20 px-3 py-1.5 border rounded-md text-sm bg-white    outline-none"
              />
              <button 
                onClick={() => {
      ...
      <div className="p-6">
        {activeTab === "overview" && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="p-4 rounded-lg bg-gray-50  border border-gray-100 ">
                <p className="text-sm text-gray-500 ">{periodFilter} Budget Total</p>
                <h3 className="text-2xl font-bold text-gray-900 ">
        ...
        {activeTab === "expenses" && (
          <div className="space-y-4">
            ...
            {showExpenseForm && (
              <form onSubmit={handleCreateExpense} className="p-4 bg-gray-50  rounded-md space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm mb-1 text-gray-700 ">Amount ({selectedCurrency})</label>
                    <input required type="number" step="0.01" value={expenseAmount} onChange={e => setExpenseAmount(e.target.value)} className="w-full px-3 py-2 border rounded-md   " />
                  </div>
                  <div>
                    <label className="block text-sm mb-1 text-gray-700 ">Date</label>
                    <input required type="date" value={expenseDate} onChange={e => setExpenseDate(e.target.value)} className="w-full px-3 py-2 border rounded-md   " />
                  </div>
                  <div className="md:col-span-2">
                    <label className="block text-sm mb-1 text-gray-700 ">Description</label>
                    <input type="text" value={expenseDescription} onChange={e => setExpenseDescription(e.target.value)} className="w-full px-3 py-2 border rounded-md   " />
                  </div>
                </div>
        ...
        {activeTab === "budgets" && (
          <div className="space-y-4">
            ...
            {showBudgetForm && (
              <form onSubmit={handleCreateBudget} className="p-4 bg-gray-50  rounded-md space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm mb-1 text-gray-700 ">Name</label>
                    <input required type="text" value={budgetName} onChange={e => setBudgetName(e.target.value)} className="w-full px-3 py-2 border rounded-md   " />
                  </div>
                  <div>
                    <label className="block text-sm mb-1 text-gray-700 ">Amount ({selectedCurrency})</label>
                    <input required type="number" step="0.01" value={budgetAmount} onChange={e => setBudgetAmount(e.target.value)} className="w-full px-3 py-2 border rounded-md   " />
                  </div>
                  <div className="md:col-span-2">
                    <label className="block text-sm mb-1 text-gray-700 ">Period</label>
                    <select value={budgetPeriod} onChange={e => setBudgetPeriod(e.target.value as BudgetPeriod)} className="w-full px-3 py-2 border rounded-md   ">
                      <option value="WEEKLY">Weekly</option>
                      <option value="MONTHLY">Monthly</option>
                      <option value="YEARLY">Yearly</option>
                    </select>
              ...
              {filteredBudgets.map(b => (
                <div key={b.id} className="p-4 border rounded-lg  flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-2xl font-bold mt-2 text-gray-900 ">{b.currency} {b.amount.toLocaleString()}</p>
